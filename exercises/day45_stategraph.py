...
def docs(state: State) -> dict:
    print("  [docs]     answer = (would call rag_service.answer_question)")
    return {"answer": "(would call rag_service.answer_question)"}


def classify(state: State) -> dict:
    if "price" in state["question"].lower():   # check the user's question
        route = "price"
    elif "greet" in state["question"].lower():
        route = "greet"
    else:
        route = "docs"
    print(f"  [classify] route = {route}")      # show what this node decided
