...
**Session addendum — 2026-09-01 (Day 37)**

- Day 37: async agent. Three cases MEASURED, not argued. (A) two INDEPENDENT `get_price` calls in one round → both START t+1.57, both END t+3.57: 4s of sleep in 2.0s of wall clock. (B) DEPENDENT chain name→ticker→price → `get_ticker` 0.77-2.77, `get_price` 3.48-5.48, zero overlap; `gather` can only start calls that EXIST, and `get_price(ticker=?)` doesn't exist until `get_ticker` returns. **Async overlaps WAITING, never CAUSALITY.** (C) two SEPARATE agent runs: 13.8s serial vs 6.9s via `asyncio.gather` — exactly 2x, and the only reason to go async at all (FastAPI + `agent.ainvoke` = N coroutines on one loop, not N blocked threads).
- Read from his own `.venv`, not the docs: `ToolNode._func` (sync, `.invoke`/`.stream`) uses `executor.map` — a THREADPOOL; `_afunc` (async) uses `await asyncio.gather` — one thread. Both parallelize tool calls; async's win is not burning an OS thread per blocking wait. Also: `langchain/tools/tool_node.py` is a 3-line re-export of `langgraph.prebuilt.tool_node` → LangChain is the model/tool layer, LangGraph is the runtime that owns the loop.
- UNPROMPTED HYPOTHESIS TEST (his, third session running where an explanation became an experiment): he noticed `part_a` and `part_b` were byte-identical except one string, predicted that pasting B's question into A would yield B's timing, ran it, confirmed. Generalised: **concurrency is a property of the REQUEST shape, not of the code.** The lever he controls is tool design — a batch `get_prices(tickers: list[str])` collapses 10 round trips to 1.
- Closed Day 36's deferred question with data: the "silent gaps" were discarded `input_json_delta` chunks carrying `partial_json` (streamed tool ARGUMENTS). Unfiltered, gaps collapsed to ~0.6s except two genuine ~5s API stalls. The filtered instrument was reporting the filter. Companion finding: wall clock swung 4.4s → 8.6s across IDENTICAL Part A runs while the tool interval stayed 2.00s every time — model latency is the noise.
- Quiz 3 asked. Q1 (updates vs values count) and Q2 (ship decision) got definitions/descriptions but not the answer — completed by Claude per the rule, no follow-ups. Q3 cold menu-vs-trips **REGRESSED**: correct transcript trace, wrong cause ("number of tools (2)"). Corrected in place; Day 37 Part B then measured the correction. Weak spot REOPENED — re-ask cold in 2-3 sessions with 3 tools and 2 dependent steps so count and depth can't be confused.
- PROCESS NOTE, CLAUDE'S GAP: he stopped twice with "why are we doing this / what are we going to prove?" — both times the instrument (the `stamp()`, the fake `asyncio.sleep`) arrived before its purpose. New rule: every non-obvious piece of scaffolding gets ONE sentence of purpose BEFORE the code — what claim it tests and what the two possible outcomes look like. Once explained, he engaged hard and generated the hypothesis test himself.
- Next: Day 38 — LlamaIndex vs LangChain (last untouched Phase 2 item): rebuild the existing retrieval in LlamaIndex over the same chunks + a 5-line "when I'd pick which" verdict.

**Session addendum — 2026-09-02 / 2026-09-04 (Day 38 + Day 38b)**

- Day 38 Part A (09-02): rebuilt the existing RAG in LlamaIndex over the IDENTICAL 6 chunks and the same MiniLM embedder -> same top chunk, same ordering (`MATCH: True`). The framework buys CODE, not better retrieval. Findings: `score = math.exp(-distance)` (chroma/base.py:472) so `THRESHOLD=1.2` becomes `score > 0.301` and constants do NOT port across frameworks; LlamaIndex embeds METADATA with the text by default (`MetadataMode.EMBED`), inflating doc4's distance 0.128 -> 0.365 non-uniformly until `excluded_embed_metadata_keys` fixed it (diagnosis confirmed by PREDICTING the exact number); `chat_content_qa_template` silently replaced his SYSTEM_PROMPT including the `refused=True` contract; `refine` = one LLM call per chunk, sequential.
- Day 38 SESSION ENDED EARLY on burnout — "I am burning out. Can't understand this LlamaIndex. I think I am lost." Cause was CLAUDE'S: two back-to-back `.venv` internals dives, both chasing a discrepancy Claude's own example introduced. New CONTENT DENSITY protocol: at most ONE library-internals dive per session; teach a framework's SHAPE before its footnotes. Two other rules came out of the same session: anchor analogies in Node/TypeScript or C#, NOT Java/Spring ("I am not from Java"); and PASTE his own code inline when referencing his files.
- Day 38b (09-04): short recovery session, protocols honored — shape first (`Document -> Node -> Index -> Retriever -> QueryEngine` in one screen, EF Core analogy), no `.venv` at all. Part B ran green: a 768-wide vector into the 384-wide collection -> `InvalidArgumentError` FROM CHROMA. The store owns the embedding contract; the framework has no opinion. Quiz 3/3 cold again (second session running), all three answered at mechanism level, nothing to complete.
- HIS UNPROMPTED ADDITIONS this session: (a) traced the failure forward — unpinned, `Settings.embed_model` defaults to OpenAI at 1536, so the unpinned script IS that error; (b) named the parity fix for the missing threshold himself — `SimilarityPostprocessor(similarity_cutoff=0.301)`, using the number the script already computes. Fifth session running where an explanation becomes his own experiment or extension.
- CARRIED FORWARD from this session: the QUIET twin of Part B (same 384 width, different vector space — `bge-small-en-v1.5` vs MiniLM — no error, silently wrong neighbours) is named but NOT tested. Also still open: `.vscode/launch.json` with PYTHONPATH (creation unconfirmed), the `time.sleep(2)` deletion in day36, the batch-tool extension from Day 37, and the Phase 1 recap out loud (owed since 2026-08-08).
- Next: Day 39 — Project 2 hardening I (real Autodesk chunks through ingest.py). EXTERNAL DEPENDENCY: a real Autodesk corpus must be staged before the session starts.

**Session addendum — 2026-09-04 (Day 39)**

- Day 39: Project 2 hardening I — the toy corpus finally replaced. EXTERNAL DEPENDENCY was unmet at session start (no real corpus existed; `ingest.py` still held 6 hardcoded one-liners), so Claude fetched 7 real Autodesk Revit help pages into `corpus/revit_help/` + `manifest.json` (1,734 words vs ~90). Caveat recorded: two pages came back partly paraphrased by the fetch — fine as corpus, not citable.
- He wrote `ingest_corpus.py` (repo root, so `from retriever import client` works with no `sys.path` bootstrap — the fix for Day 38's VS Code import breakage) and `audit_corpus.py`. 18 chunks into a NEW collection `revit_docs_v2`; `revit_docs_project_2` untouched so the comparison is data, not memory.
- TWO OF CLAUDE'S HYPOTHESES FALSIFIED BY HIS RUNS IN ONE SESSION: (1) "real prose will collapse the distance band and break THRESHOLD=1.2" — WRONG, hits 0.57-0.84 vs miss 1.48-1.62, the gate held; (2) "long chunks rank worse through dilution" — WRONG, the 2,623-char chunk ranked 5th of 8, above three shorter ones.
- THE ACTUAL FINDING (the best of the session): PRECISION failure with every guard silent. "How do I create a wall?" ranked a footnote about auditing project files FIRST (0.619) and a doors chunk SECOND (0.827); the answering chunk ranked 7th (1.106). Distance gate PASSES, `refused=True` never fires, RAGAS faithfulness would score high — the answer is faithfully grounded in the WRONG chunk. Model: vector search ranks ABOUTNESS, not ANSWERHOOD; a threshold measures COVERAGE and can never measure PRECISION.
- Quiz 3/3 for the FOURTH consecutive session, nothing needing completion. Q2 in particular ("the store accepts every write, and retrieval silently rots — distances still computed, still ranked, no longer meaning anything") was the quiet-failure case answered cold before it was ever taught.
- CARRIED FORWARD from Day 39: (a) retrieval precision eval with EXPECTED CHUNK IDS — the instrument he has never had; (b) hard-bounded chunker (split inside an oversized paragraph); (c) the title-prepend experiment left as his call in `ingest_corpus.py`; (d) `category` metadata is written but unused by the audit; (e) reranking as the real precision fix (Phase 3).
- Next: Day 40 — Project 2 hardening II (model cost decision measured not guessed, ragas upgrade, ONE paid ragas_evals.py run). EXTERNAL DEPENDENCY: the paid run must be budgeted.

**Session addendum — 2026-09-08 (Day 40)**

- PLAN CHANGE, Claude's recommendation and his go: Day 40's planned model-cost decision + paid ragas run had an unmet budget dependency, so it was swapped for the retrieval precision eval (Day 39's biggest carried-forward item) — $0, deterministic, no LLM. Day 41 takes the deferred work; PHASE 2 CLOSE moves to Day 42, still inside September.
- He wrote `precision_eval.py` at repo root: chunk inventory -> labelled `CASES` (question -> expected chunk ids, `[]` = nothing should answer it) -> rank loop at DEPTH=10 -> hit@2 / hit@10 / MRR -> hit@k sweep.
- THE HEADLINE, and it is a lesson about instruments, not about Chroma: his first label set labelled by SOURCE DOCUMENT (every chunk from each file), which INCLUDED `about_walls_2` — the auditing footnote that ranked 1st on Day 39 — and EXCLUDED `walls_overview_0`. Scored against those labels, Day 39's bug reads as hit@1 = TRUE, precision 100%. The label set IS the test. Honest rule adopted: "if a reader got ONLY this chunk, could they do the thing?"
- MEASURED: hit@10 = 1.000, hit@2 = 0.750 — retrieval is not broken, RANKING is. The wall answer sits at 1.082, UNDER `THRESHOLD = 1.2`; `N_RESULTS = 2` is what excludes it. THRESHOLD is a quality gate, N_RESULTS is an admission gate, and only the first had ever been measured. hit@k sweep: flat 0.750 for k=2..5, 1.000 at k=6 — widening is a cliff, not a dial, which is the measured case for reranking in Phase 3.
- Quiz 3/3 for the FIFTH consecutive session. Q2 specified the entire day's exercise cold, before it was taught: labelled expected chunk IDs, depth past N_RESULTS, record the RANK, `retrieve()` direct with no Claude in the loop.
- CLAUDE'S OWN MISS, recorded: the re-label was asked twice and failed twice (second paste unchanged plus a copy slip). Correct move was to hand over the finished CASES block after ONE worked example. Labelling judgment gets taught once with a worked table, then delivered.
- CARRIED FORWARD from Day 40: (a) grow CASES to 15-20 questions — at n=4 each question is 25 points; (b) decide `N_RESULTS` 2 -> 6 as a measured cost/precision trade; (c) reranking stays Phase 3, now with his own hit@k cliff as the argument; (d) Day 39's chunker hard-bound, title-prepend experiment, and unused `category` metadata all still open.
- Next: Day 41 — Project 2 hardening II. EXTERNAL DEPENDENCY: the paid ragas run must be budgeted BEFORE the session; it went unmet on Day 40 and cost the day's original plan.

**Session addendum - 2026-09-09 (Day 41)**

- DEPLOY, part 1. THE HEADLINE: production had been serving the 6-chunk TOY corpus the whole time - `retriever.py` hard-coded `revit_docs_project_2`, so Days 39-40's corpus and precision work in `revit_docs_v2` had never been on the request path. Blue/green worked as designed on Day 39; the POINTER FLIP was the step nobody wrote down. Fixed as `COLLECTION_NAME = os.environ.get("REVIT_COLLECTION", "revit_docs_v2")`.
- The vector DB is DERIVED data (`revit_db_07_25/` gitignored); the corpus is source (`corpus/revit_help/` tracked), so the deploy build step is `python ingest_corpus.py`, proven from empty (0.720/0.756 reproduced to 3 decimals). The first "passing" rebuild was caught as a test that never ran - the `mv` had not happened and identical distances looked like success.
- Serving deps measured by `sys.modules` after a live `retrieve()`, not guessed: 1.8GB dev venv, 7 packages actually imported. Chroma embeds with ONNX MiniLM at query time, so torch/transformers are dead weight; `onnxruntime` pinned deliberately because it IS the embedder.
- 3/3 quiz (sixth running); 4 predictions stated before running, 4 matched. `READINESS.md` written after his "when do I become a good AI engineer?" - 9-row scorecard, six held, estimate Jan-Feb 2027.
- Claude's miss: gave a Render Start Command in a table labelled "paste these exactly" and he ran `uvicorn --port $PORT` in his own shell where `$PORT` is unset. Deploy-target commands must be labelled with WHERE they run.

**Session addendum - 2026-09-10 (Day 42)**

- DEPLOY HARDENING, all local; the Render service was NOT created and 69fe01d is NOT pushed. Both carry to Day 43.
- `/health` added to `app.py`: collection, `count()`, db_path, embedder class, `chromadb.__version__`, THRESHOLD, N_RESULTS - read from the SAME `collection` handle `retrieve()` queries. The lesson: `/heartbeat` returned OK on every day production served the toy corpus, so liveness certifies nothing about identity.
- `refused` now crosses the HTTP boundary (it had lived in `RagResponse` since Day 40 while `AskResponse` shipped only `answer` + `sources`, forcing HTTP clients to string-match the refusal prose - the exact thing his own comment at `rag_service.py:16` forbids). One `{code, message}` envelope now covers 422 BAD_REQUEST (empty question) and 502 UPSTREAM_ERROR (Claude raises); a second handler on `HTTPException` is what keeps the 502 from arriving wrapped in `detail`.
- HIS CATCH, and it is the win of the day: after the rewrite he asked "we didn't ask correct question?" - we had proven both FAILURE paths and never re-run the SUCCESS path, so `refused:false` was untested code. He audited the TEST PLAN, which was mine, not his code.
