...
TOOLS = [{
    ...
    "description": "Get the ticker symbol for a given company name. Use FIRST when the user names a company instead of giving a ticker.",
    "input_schema": {
        "type": "object",
        "properties": {
            "company_name": {"type": "string", "description": "Company name, e.g. 'YUNextGenAI'"},
        },
...
PRICES = {"AAPL": 189.50, "MSFT": 402.10, "YNXT": 42.00}
NAMES = {"AAPL": "Apple Inc.", "MSFT": "Microsoft Corp."}
TICKERS = {"YUNextGenAI": "YNXT"}
...
def get_ticker_symbol(company_name):
    if company_name not in TICKERS:
        raise ValueError(
            f"unknown company '{company_name}' — known: {list(TICKERS)}")
    return TICKERS[company_name]


TOOL_FUNCTIONS = {
    "get_stock_price": get_stock_price_strict,
    "get_company_name": get_company_name,
    "get_ticker_symbol": get_ticker_symbol
}   # dynamic dispatch
...
INPUT_MODELS = {
    ...
    "get_company_name": StockPriceInput,
...
def give_up(messages):
    ...
    return client.messages.create(model="claude-sonnet-5", max_tokens=500, messages=messages)
...
final = run_agent("What's the current stock price of YUNextGenAI?")
